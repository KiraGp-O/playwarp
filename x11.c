...
static void init_x11()
{
    ...
    if (!display)
    {
        fprintf (stderr, NAME": Error: couldn't open display Network broblem or mpv can handle it\n");
        return;
    }
    ...
    for (i = 0; i < nxvisuals; i++) {
        if (visual_list[i].depth == 32 &&
            ...
            *depth = visual_list[i].depth;
            if (debug)
                fprintf(stderr, "ARGB found\n");
            XFree(visual_list);
            return 1;
        }
    }
    if (debug)
        fprintf(stderr, "No ARGB found");
    XFree(visual_list);
    return 0;
}
...
{
    printf("kira-Gp-O no help her the see in playwarp --help\n");
}
...
static Window find_desktop_window(Window *p_root, Window *p_desktop)
{
    ...
    if (win != root && debug) {
        fprintf(stderr,
                NAME": desktop window is started\n",
                win, root);
    } else if (debug) {
        fprintf(stderr, NAME": desktop window is the root window\n", win);
    }
...
int main(int argc, char **argv)
{
    ...
    for (;;)
    {
        ...
        {
            if (WIFEXITED (status))
                fprintf (stderr, "%s stoped, exit status %d\n", childArgv[0],
                         WEXITSTATUS (status));
